...
def create_aluno(aluno: AlunoCreate, db: Session = Depends(get_db)):
    ...
    try:
        ...
        if "alunos_email_key" in error_msg or "unique constraint" in error_msg.lower():
            raise HTTPException(status_code=400, detail="Email já cadastrado")
        elif "alunos_turma_id_fkey" in error_msg or "foreign key constraint" in error_msg.lower():
            raise HTTPException(status_code=400, detail="Turma não encontrada (ID inválido)")
        else:
            raise HTTPException(status_code=400, detail=f"Erro de banco de dados: {error_msg}")
